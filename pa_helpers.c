...
/*
New Index functions, tracks the ones that are used and such.
*/

void ModelIndex_P (char *name)
{
	if (!level.time)
	{
		gi.modelindex (name);
	...
	strcpy (modelindexes[modelindexes_l], name);
	modelindexes++;

	gi.dprintf ("%s %s\n", name, modelindexes[modelindexes_l]);
}